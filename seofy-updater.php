<?php
class WP_GitHub_Updater_For_SeofyPlugin_Dk_Navigation_Health {
  ...
  /**
    ...
    function check_for_plugin_update($transient) {
        ...
        $plugin_slug = plugin_basename(SDKNH_PLUGIN_DIRECTORY);
       
        $plugin_data = get_plugin_data(SDKNH_PLUGIN_DIRECTORY);
      
        $github_url = SDKNH_GITHUB_URL;

        
        $request = wp_remote_get($github_url, array(
            'headers' => array(
                'Authorization' => 'token ' . SDKNH_GITHUB_TOKEN,
            ),
        ));
        ...
        if (isset($request[0]) && version_compare($plugin_data['Version'], $request[0]->tag_name, '<')) {
            ...
            $transient->response[$plugin_slug] = (object) array(
                'slug' => SDKNH_PLUGIN_DIRECTORY,
                'plugin' => SDKNH_PLUGIN_SLUG,
    ...
    function http_request_args_filter($args, $url) {
        if (strpos($url, 'https://api.github.com/') !== false) {
            $args['headers']['Authorization'] = 'token ' . SDKNH_GITHUB_TOKEN;
        }
    ...
    public function upgrader_post_install( $true, $hook_extra, $result ) {
        ...
        // Move & Activate
        $proper_destination = WP_PLUGIN_DIR.'/'.SDKNH_PROPER_FOLDER_NAME;
        $wp_filesystem->move( $result['destination'], $proper_destination );
        $result['destination'] = $proper_destination;
        $activate = activate_plugin( WP_PLUGIN_DIR.'/'.SDKNH_PLUGIN_SLUG );
    ...
    function is_plugin_update($plugin_info) {
        ...
        return $directory === dirname(SDKNH_PLUGIN_DIRECTORY);
    }
